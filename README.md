# DBProj3 - Recipe Database Web Application
...
All features described in the original proposal were implemented.

- **View Recipes:** Display all recipes with their name, portion size, and source
- **Manage Households:** Add and delete households from the system
- **Inventory Management:** Track ingredients in each household's inventory with quantities and units
- **Cookable Recipes:** Identify which recipes can be made based on a household's current inventory
- **Meal Plans:** Create, view, and delete meal plans for households with automatic grocery list generation
- **Recipe Assignment:** Add multiple recipes from the database to meal plans
- **Grocery Lists:** Automatically generate grocery lists from meal plan recipes with ingredient aggregation

## Interesting Database Operations

### 1. Cookable Recipes Page (`/cookable`)

**Purpose:** This page helps households discover which recipes they can make with their current inventory, reducing food waste and helping with meal planning decisions.
...
This operation demonstrates  SQL logic using double negation (NOT EXISTS with NOT IN). The query tries to find recipes where there does not exist any required ingredient that is not in the household's inventory. This type of operation is one of the more complex relational operations to implement in SQL.
...
### 2. Meal Plan Creation with Grocery List Generation (`/mealplans`)

**Purpose:** This page allows households to plan meals and automatically generates shopping lists with ingredient quantities.

**Database Operations:**
- **Multi-table INSERT:** Creates records across 4 related tables in a single transaction:
...
5. Access the application at `http://localhost:8111` or VM URL
